...
with open("./proxyattention/pickler.pkl", "rb+") as f:
    ...
    model, target_layers, input_wrong, label_wrong = orig

#%%
input_wrong.shape, label_wrong.shape
#%%
label_wrong
#%%
...
#%%
grads = torch.Tensor(grads).to("cuda").unsqueeze(1)
#%%
grads = grads.expand(-1, 3, -1, -1)
#%%
thres = 0.8
#%%
input_wrong = inv_normalize(input_wrong)
#%%
thresholded_ims = torch.where(grads > thres, torch.mean(grads), input_wrong)
#%%
tfm(thresholded_ims[4])


#%%
randtest = torch.zeros(1, 3, 224, 224)
randtest
# %%
inv_normalize(randtest)